...
def create_gradient_material():
    ...
    # Create material
    mat = bpy.data.materials.new(name="Infinity_Gradient")
    mat.use_nodes = True
    ...
    # Set material properties (Blender 4.3.2 compatible)
    if hasattr(bsdf.inputs, 'Specular'):
        bsdf.inputs['Specular'].default_value = 0.5
    if hasattr(bsdf.inputs, 'Subsurface Scattering'):
        bsdf.inputs['Subsurface Scattering'].default_value = 0.1

    bsdf.inputs['Base Color'].default_value = (0.0, 0.5, 0.8, 1.0)  # Base blue color
    bsdf.inputs['Metallic'].default_value = 0.3
    bsdf.inputs['Roughness'].default_value = 0.2

    # Add Texture Coordinate and Gradient Texture for more interesting effect
...
def setup_scene():
    """Setup the scene with lighting and camera"""
    ...
    bg_node.inputs['Color'].default_value = (0.95, 0.95, 0.95, 1.0)  # Light gray
...
def setup_render_settings():
    ...
    scene.render.resolution_percentage = 50  # 50% for faster processing
